 ...
 * Работа с товарами
 *
 * @copyright 	2011 UPCMS
 * @link 		http://upcms.ru
 * @author 		UPCMS
 *
 */

require_once('UPcms.php');

class Products extends UPcms
{
	/**
	...
	public function get_products($filter = array())
	{
		...
		if(isset($filter['visible']))
 		...
 		if(!empty($filter['sort']))
			...
			{
				case 'position':
				$order = ' (SELECT v.price FROM __variants v WHERE v.product_id = p.id) > 0 DESC, p.position ASC';
				break;
				case 'positiondesc':
				$order = ' (SELECT v.price FROM __variants v WHERE v.product_id = p.id) > 0 DESC, p.position DESC';
				break;
				case 'name':
				$order = '(SELECT v.price FROM __variants v WHERE v.product_id = p.id) > 0 DESC, p.name ASC';
				break;
				case 'namedesc':
				$order = '(SELECT v.price FROM __variants v WHERE v.product_id = p.id) > 0 DESC, p.name DESC';
				break;
				case 'created':
				...
				$order = '(SELECT v.price FROM __variants v WHERE v.product_id = p.id) > 0 DESC, (SELECT -pv.price FROM __variants pv WHERE (pv.stock IS NULL OR pv.stock>0) AND p.id = pv.product_id AND pv.position=(SELECT MIN(position) FROM __variants WHERE (stock>0 OR stock IS NULL) AND product_id=p.id LIMIT 1) LIMIT 1) ASC';
				break;
				case 'pricedesc':
				//$order = 'pv.price IS NULL, pv.price=0, pv.price';
				$order = '(SELECT v.price FROM __variants v WHERE v.product_id = p.id) > 0 DESC, (SELECT -pv.price FROM __variants pv WHERE (pv.stock IS NULL OR pv.stock>0) AND p.id = pv.product_id AND pv.position=(SELECT MIN(position) FROM __variants WHERE (stock>0 OR stock IS NULL) AND product_id=p.id LIMIT 1) LIMIT 1) DESC';
				break;
			}
        ...
        {
            $price_from_filter = empty($filter['price_from']) ? '' : $this->db->placehold("AND price >= ?", $filter['price_from']);
            $price_to_filter = empty($filter['price_to']) ? '' : $this->db->placehold("AND price <= ?", $filter['price_to']);
            $prices_filter = $this->db->placehold("AND p.id IN (SELECT product_id FROM __variants WHERE 1 $price_from_filter $price_to_filter)");
        }
        ...
        if ($results = $this->db->results())
        {
            foreach ($results as &$r)
            {
                if (!empty($r->documents))
                    $r->documents = json_decode($r->documents);
            }
        }
	...
	*/
	...
	public function update_product($id, $product)
	{
		...
		if (!empty($product['documents']) && is_array($product['documents']))
            $product['documents'] = json_encode($product['documents']);
	...
	*/
	...
	public function delete_image_byFilename($filename)
	{
		$query = $this->db->placehold("DELETE FROM __images WHERE filename=? LIMIT 1", $filename);
		$this->db->query($query);
		$query = $this->db->placehold("SELECT count(*) as count FROM __images WHERE filename=? LIMIT 1", $filename);
		$this->db->query($query);
		$count = $this->db->result('count');
	...
	*/
	...
	public function get_min_max($filter = array())
    {
        $product_id_filter = '';
        $category_id_filter = '';
        $brand_id_filter = '';
        ...
        if(!empty($filter['id']))
			$product_id_filter = $this->db->placehold('AND v.product_id in(?@)', (array)$filter['id']);

		if(!empty($filter['category_id']))
			$category_id_filter = $this->db->placehold('AND v.product_id IN (SELECT product_id FROM __products_categories WHERE category_id IN (?@))', (array)$filter['category_id']);
		...
		if(!empty($filter['discounted']))
			$discounted_filter = $this->db->placehold('AND v.compare_price>0 ');
        ...
        if(!empty($filter['keyword']))
		{
			...
			foreach($keywords as $keyword)
			{
				$kw = $this->db->escape(trim($keyword));
				if($kw!=='')
					$keyword_filter .= $this->db->placehold("AND (p.name LIKE '%$kw%' OR p.meta_keywords LIKE '%$kw%' OR v.sku LIKE '%$kw%')");
			}
		}
...
//echo '<hr />'.__FILE__.':'.__LINE__.'<pre>'; var_dump($query); echo '</pre>';
        return $result;
    }
}
